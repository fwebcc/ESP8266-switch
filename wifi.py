...
def do_connect():
          ...
          print('链接失败无网络', wlan.ifconfig(),wlan.config('dhcp_hostname'))
def GO_wifi():
       if not wlan.isconnected():
          do_connect()
       else:
          if setting['MQTT_Topic'] ==  wlan.config('dhcp_hostname'):
              print('链接成功', wlan.ifconfig(),wlan.config('dhcp_hostname'))
          else:
              wlan.active(False)
              do_connect()
